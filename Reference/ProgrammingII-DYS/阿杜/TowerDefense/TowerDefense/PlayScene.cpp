...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		que.pop();
		auto dis = map[p.y][p.x];
		if ((p.y-1>=0)&&(p.y-1<=MapHeight-1)&&(p.x>=0)&&(p.x<=MapWidth-1)&&(mapState[p.y-1][p.x]==TILE_DIRT)&& (map[p.y - 1][p.x] == -1)) {
		...
		if ((p.y >= 0) && (p.y <= MapHeight - 1) && (p.x - 1 >= 0) && (p.x - 1 <= MapWidth-1) && (mapState[p.y][p.x-1] == TILE_DIRT) && (map[p.y][p.x - 1] == -1)) {
