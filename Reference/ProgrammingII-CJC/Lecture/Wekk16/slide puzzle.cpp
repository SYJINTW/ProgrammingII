...
template <int SIZE> class SlideGame
{
    using State = array<int, SIZE>;
private:
    State inital;          // initial state
    Node<SIZE>* sol;   // the solution
    int LEN;            // LEN = sqrt(SIZE)

public:
    // constructor
    SlideGame(State input): inital(input), sol(nullptr){
        LEN = sqrt(SIZE);
    };
    ...
    // solve the problem using BFS, return number of steps to reach the goal.
    int solve()
    {
        queue<Node<SIZE>*>  q;      // queue
        set<State>   explored;      // the states appeared before
        Node<SIZE>* current = new Node<SIZE>(inital);  // current solution
