...
namespace I2P2_test
{
    ...
    struct back_and_front
    {
        ...
        {
            if (c.size())
                return check(c.back() == s.back(), "back is not equal") & check(c.front() == s.front(), "front is not equal");
        ...
        bool operator()(T &c, Std &s) const
        {
            ...
            if (c.size())
            {
                ...
                if (std::is_same<CheckAfter, yes_tag>::value)
                    iter_check.store(next<RandomIter>()(begin_pos), c.end());
                c.erase(begin_pos);
    ...
    struct insert_key_value
    {
        ...
        bool operator()(T &c, Std &s) const
        {
            ...
            result &= iter_check.valid(0, pos, iter_begin);
            if (not_exist)
                iter_begin = next<RandomIter>()(iter_begin);
    ...
    struct pop_front
    {
        ...
        bool operator()(T &c, Std &s) const
        {
            ...
            if (c.size())
            {
                iterator_valid_check<decltype(c.begin()), decltype(std::addressof(*c.begin()))> iter_check;
                if (std::is_same<CheckAfter, yes_tag>::value)
                    iter_check.store(next<no_tag>()(c.begin()), c.end());
    ...
    struct reserve
    {
        ...
        {
            bool result(true);
            std::uniform_int_distribution<size_t> new_capacity_gen(std::max<size_t>(1, capacity<WithCapacity>()(c)) - 1, capacity<WithCapacity>()(c) + 1);
    ...
    struct iterator_iterate
    {
        ...
        bool operator()(T &&c) const
        {
            ...
            if (c.size())
            {
                ...
                result &= check(c.begin() == c.end(), "begin is not equal to end when container is empty");
            return result;
        }
    };
    ...
    void check_list()
    {
        ...
        for (unsigned long i(0); i != op_test_cnt; ++i)
        {
            ...
            case 33:
            case 34:
                std::conditional<std::is_same<WithReserve, yes_tag>::value, reserve<WithCapacity>, Empty>::type()(*usr[select], stdc[select]);
                break;
            case 35:
            case 36:
                std::conditional<std::is_same<WithShrink, yes_tag>::value, shrink_to_fit, Empty>::type()(*usr[select], stdc[select]);
                break;
            default:
                assert(false);
            }
            ...
            std::conditional<std::is_same<RandomIter, yes_tag>::value, iterator_random, Empty>::type()(*usr[select]);
